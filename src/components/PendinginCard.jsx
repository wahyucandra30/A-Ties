...
const PendinginCard = ({ data, id }) => {
    ...
    return (
        <>
            {showModal &&
                <div className="w-screen h-screen backdrop-blur-[2px] fixed z-10 top-0 left-0 flex justify-center items-center 
                bg-opacity-50 bg-gray-500">
                    <div className="bg-white rounded w-full lg:w-80 min-w-[128px] p-8 relative">
                        <X size={21} className="text-gray-400 absolute top-6 right-8 cursor-pointer"
                            onClick={() => setShowModal(false)} />
                        <div className="flex items-center text-sm text-orange-400 gap-1 leading-0">
                            <h1 className="text-lg flex items-end font-bold leading-0">
                                {data?.masalah}
                            </h1>
                        </div>
                        <h2 className="font-bold text-[14px]">Solusi Perbaikan</h2>
                        <textarea onChange={(e) => setSolusi(e.target.value)}
                            className="rounded-md w-full p-2 border-gray-300 border"
                            rows="4" cols="50" draggable="false" placeholder="Beri catatan untuk perbaikan masalah mesin pendingin. Tenaga kerja lapangan akan mendapatkan catatan ini.">
                        </textarea>
                        <button onClick={handleSolusi}
                            className="w-full bg-teal-600 rounded-md p-2 mt-2 text-white flex items-center justify-center font-bold text-[14px]">
                            Kirim
                        </button>
                    </div>
                </div>
            ...
            <div className="border border-gray-300 drop-shadow-sm min-w-[128px] lg:w-56 w-full p-4 rounded-md relative flex flex-col gap-1 justify-between">
                <div>
                    ...
                    <div className={`text-white px-4 rounded-full w-fit shadow-gray-300 mb-1
            ${data?.status === "Baik" ?
                            "bg-emerald-500" :
                    ...
                    <ul className="flex flex-col gap-1">
                        ...
                        <li className={`flex items-end gap-1 ${data?.kadaluarsa > 18 ? "text-green-500"
                            ...
                            <Wrench size={21} />
                            <abbr className={`text-sm border-none no-underline`}
                                title={data?.kadaluarsa > 0 ?
                                    `${data?.kadaluarsa} hari sebelum jadwal perawatan`
                                    :
                                    `${data?.kadaluarsa * -1} hari terlambat perawatan`}>
